...
async function importData() {
    try {
        console.log('Reading P2 position.csv...');
        let text = fs.readFileSync('P2 position.csv', 'utf8');
        ...
        console.log('Cleaning up "P2 position" table in Supabase...');
        ...
        tables.forEach(table => {
            table.rows.forEach((row, rowIndex) => {
                const dbRow = {
                    ...
                    row_index: rowIndex,
                    col0: row[0] !== undefined ? row[0] : null,
                    col1: row[1] !== undefined ? row[1] : null,
                    col2: row[2] !== undefined ? row[2] : null,
                    col3: row[3] !== undefined ? row[3] : null,
                    col4: row[4] !== undefined ? row[4] : null,
                    col5: row[5] !== undefined ? row[5] : null,
                    col6: row[6] !== undefined ? row[6] : null,
                    col7: row[7] !== undefined ? row[7] : null,
                };
                dbRows.push(dbRow);
            });
        });

        console.log(`Inserting ${dbRows.length} rows into "P2 position"...`);
        ...
        console.log('Import to "P2 position" completed successfully!');
        ...
        if (countError) {
            ...
            console.log(`Verified total count in "P2 position" table: ${count} rows`);
        }
